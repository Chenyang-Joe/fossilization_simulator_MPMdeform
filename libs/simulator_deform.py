...
class SimulatorDeform:
    ...
    def load_raw_mesh(self):
        ...
        self.vanilla_mesh = trimesh.load(self.model_path)
        print(f"#Vertices: {len(self.vanilla_mesh.vertices)}")
        print(f"#Faces: {len(self.vanilla_mesh.faces)}")


    def downsampling(self):
        # downsampling if self.faces_limitation set
        if self.faces_limitation and (self.faces_limitation <= len(self.vanilla_mesh.faces)):
            print(f"Downsampling to #Faces {self.faces_limitation}")
            o3d_mesh = o3d.geometry.TriangleMesh(
                o3d.utility.Vector3dVector(self.vanilla_mesh.vertices),
                o3d.utility.Vector3iVector(self.vanilla_mesh.faces)
            )
            simplified = o3d_mesh.simplify_quadric_decimation(target_number_of_triangles=self.faces_limitation)
            self.vanilla_mesh = trimesh.Trimesh(
    ...
    def pruning(self, min_faces = 10, deep_pruning = True):
        ...
        if deep_pruning:
            ...
            self.vanilla_mesh.remove_duplicate_faces()
            self.vanilla_mesh.remove_degenerate_faces()
            self.vanilla_mesh.remove_unreferenced_vertices()
    ...
    def preprocess(self):
        ...
        self.pruning()
        self.rescale()
        # self.vanilla_mesh.export("./dev/vanilla_mesh.obj")
    ...
    def generate_PC(self):
        ...
        self.vanilla_mesh.ray = RayMeshIntersector(self.vanilla_mesh)
        if isinstance(self.vanilla_mesh.ray, RayMeshIntersector):
            print("Pyembree acceleration turned on")
        else:
            print("Pyembree acceleartion did not turn on")

        inside = self.vanilla_mesh.contains(grid_points)
    ...
    def select_region(self):
        ...
        mesh_rescaled = self.vanilla_mesh.copy()
    ...
    def Mesh_reconstruction(self):
        ...
        v_vanilla = np.asarray(self.vanilla_mesh.vertices)

        print("Mesh bounding box extents: ", self.vanilla_mesh.bounding_box.extents)
        ...
        self.final_mesh = trimesh.Trimesh(vertices=v_deformed, faces=self.vanilla_mesh.faces)
        save_name = os.path.join(self.save_folder, self.model_path.split("/")[-1].split(".")[0] + "_deformed.obj")
        self.final_mesh.export(save_name)
